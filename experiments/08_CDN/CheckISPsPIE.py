#!/usr/bin/env python3
"""
Experiment 08.2 — PIE Karachi: PTCL + Nayatel → Connect Communications
========================================================================
Tests whether PIE Karachi peering LAN (58.181.127.x) appears
when tracing between two PIE Karachi members: PTCL and Connect Communications.

Run from repo root:
    python3 experiments/08_CDN/exp08_2_pie_connect.py
"""
...
RUN_NAME = "run_pie_karachi_connect_comms"
...
TARGETS = [
    {"ip": "115.42.64.1",   "label": "Connect-Comms-1", "asn": "132165"},
    {"ip": "113.203.207.1", "label": "Connect-Comms-2", "asn": "132165"},
]

IXP_LAN      = "58.181.127."
IXP_NAME     = "PIE Karachi"
PTCL_PIE_IP  = "58.181.127.1"
CONN_PIE_IP  = "58.181.127.10"
...
def analyse(probe_desc, target, raw):
    ...
    for result in raw:
        ...
        for hop in result.get("result", []):
            ...
            if ip == CONN_PIE_IP:      note = "<<< Connect Comms PIE router"
...
def main():
    print("=" * 60)
    print(f"  Experiment 08.2 — {IXP_NAME}: PTCL + Nayatel → Connect Communications")
    print("=" * 60)
    print(f"\n  Watching for {IXP_NAME} LAN ({IXP_LAN}x)")
    print(f"  PTCL PIE IP:             {PTCL_PIE_IP}")
    print(f"  Connect Communications:  {CONN_PIE_IP}")
    ...
    with open(ROUTES_FILE, "w") as f:
        f.write(f"Exp 08.2 — {IXP_NAME} Connect Comms — {TIMESTAMP}\n" + "\n".join(all_routes))
