#!/usr/bin/env python3
"""
Experiment 08.3 — PKIX Lahore: Zcom + Nayatel → Transworld
============================================================
Tests whether PKIX Lahore peering LAN (100.128.0.x) appears
when Zcom (a PKIX Lahore member) traces to Transworld (also a member).

Run from repo root:
    python3 experiments/08_CDN/exp08_3_pkix_lahore.py
"""
...
RUN_NAME = "run_pkix_lahore_zcom_transworld"

PROBES = [
    (7613,    152605, "Pakistan", "Zcom Lahore"),    # PKIX Lahore member
    (60223,   23674,  "Pakistan", "Nayatel ISB"),    # not at PKIX Lahore — comparison
]

TARGETS = [
    {"ip": "110.93.215.1", "label": "Transworld-1", "asn": "38193"},
    {"ip": "110.93.250.1", "label": "Transworld-2", "asn": "38193"},
]

IXP_LAN  = "100.128.0."
IXP_NAME = "PKIX Lahore"
...
def main():
    print("=" * 60)
    print(f"  Experiment 08.3 — {IXP_NAME}: Zcom + Nayatel → Transworld")
    print("=" * 60)
    print(f"\n  Watching for {IXP_NAME} LAN ({IXP_LAN}x)")
    print(f"  Zcom (AS152605) and Transworld (AS38193) are both {IXP_NAME} members")
    ...
    with open(ROUTES_FILE, "w") as f:
        f.write(f"Exp 08.3 — {IXP_NAME} Zcom→Transworld — {TIMESTAMP}\n" + "\n".join(all_routes))
