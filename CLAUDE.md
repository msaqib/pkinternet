...
| `experiments/06.1.1_smw5_robustness/` | Exp 6.1.1: **robustness programme for 6.1's caveats** — churn-anomaly baseline (operators, 15 May–10 Jul, day-over-day churn distribution + z-score of the fault days), population-scale downstream scan (all PK origins: who switched majority gate 1–2 Jul), planned: placebo operators, fault-onset clocks (PK anchor builtins/IODA/Radar), APNIC user-weighting. `robustness.py baseline|population`. |
