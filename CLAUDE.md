...
| `experiments/06.1.1_smw5_robustness/` | Exp 6.1.1 (**done**): robustness programme for 6.1's caveats, all closed. Placebo test (4 controls, `robustness.py baseline\|population\|placebo`) retracted the churn-*magnitude* claim as a global IHR artifact but confirmed a Hurricane-swing fingerprint 2–8× larger in SMW5-exposed operators; APNIC user-weighting: 99.63% of ~42M PK users held their gate; RIPE anchor mesh independently confirms the fault's 1 Jul onset (78σ RTT/loss spike, ~1,000 external vantages). Raw pulls in top-level `data/` (`apnic_aspop_pk_*`, `ioda_pk_*`, `ripe_anchor_rtt_agg_*`). |
