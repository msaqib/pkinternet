...
%and weighted average latency for Pacific island nations fell by $64$\% once a hybrid IXP was established~\cite{lui2021costings}.
...
% That calculus is not hypothetical. For Site Z, the domestic alternative is not inferred but measured: one operator reaches it without leaving the country while six do not, so the cost of the detour is the difference between two paths that both exist today, though it is a single site rather than prevalence evidence. For the remaining tromboned pairs there is no local round to compare against at all; there, the best RTT any other Pakistani ISP achieves to that same destination sets the same bound a different way, the same best-achievable-RTT construction \S\ref{sec:ratio} used for CDN sites, applied here to domestic content instead of CDN content. A same-path comparison where one exists, a cross-ISP comparison where it does not, and both point to the same conclusion: the cost of not peering is not a rounding error.
...
Three limitations bound these claims. First, our vantages are fixed-broadband RIPE Atlas probes, while most of Pakistan's $150$ million users connect over mobile networks we have no vantage on; mobile-core routing may differ. %, and the heterogeneity we measure among fixed ISPs alone is itself a warning against extrapolating. 
...
%Fourth, our measurements are IPv4-only; a CDN poorly peered over IPv4 may have a better-peered IPv6 path, and with national IPv6 adoption still near $25$\%~\cite{pta2026telecom}, some of the CDN unevenness we measure may partly reflect uneven IPv6 rollout rather than IPv4 peering alone.
