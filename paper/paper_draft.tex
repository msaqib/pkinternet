...
that the penalty is structural, varying under 3 percentage points across the day; and that for a
third of the probe--site pairs that ever hairpin, the hairpin is confirmed not just by latency but
by a change in the actual route, and those same pairs are also served over a domestic route
elsewhere in the week---route-level evidence that the detour is a choice, not a necessity, for at
least this population. The finding of the study is
...
PKIX's own published figures show inter-ISP latency dropping from 100--144\,ms internationally to 1--31\,ms through the exchange~\cite{pta2026}. And yet, across a week of hop-level measurement from inside the country, not one of 222,944 traceroutes to Pakistani destinations crosses either of its peering fabrics. The exchange is built, connected, and capable. It is simply not being used for most of the traffic that could benefit from it.
...
Opalinski et al., in AINTEC~2024 work on this same country's physical
layer~\cite{opalinski2024quest}, extend this account and show it is not confined to commercial
incentives alone: Pakistan's
...
Singapore, is frequently unannounced in BGP and carries no reliable country at all. This is not an
edge case: of the 143 distinct Transworld routers our traces traverse, 125 answer from address
space unannounced in BGP and resolvable only through registry records---the operator's domestic
backbone is effectively invisible to control-plane measurement, which is itself a reason this
study is conducted in the data plane. We therefore
...
$+15.3$\,ms across the 121 such pairs, a heavily skewed distribution. Stated directly, against
the paper's own emphasis: for the flapping majority the detour itself is nearly
...
persistently local, 40 (9\%) are persistently hairpinned, and 193 (43\%) \emph{flap} by verdict:
the RTT-physics classifier returns a different call on some rounds than others for the same
probe--site pair. A flipped verdict alone is not sufficient evidence that the underlying route
changed---our classifier decides from latency, and latency has ordinary jitter---so we checked the
actual hop-level path (ASN sequence, not just RTT) behind every flip. Of the 233 pairs that ever
receive a hairpin verdict, \textbf{77 (33\%) show at least one round where the route itself is
confirmed to differ}---a different sequence of networks carrying the traffic, not merely a
slower reading on the same path; for the remaining 156 pairs, every hairpin verdict they ever
receive coincides with an unchanged route, consistent with threshold noise rather than rerouting.
Within the confirmed-genuine population, the same conclusion RQ3 asks for holds: these pairs are
also served over a domestic route elsewhere in the week, direct route-level evidence that the
detour is a choice, not a necessity, for that population. The choice extends to the gate itself:
23 pairs were handed to \emph{both} PTCL and Transworld at different times in the week for the
same destination---an unambiguous case, since the two carriers are different ASNs. Hop-level
analysis of the confirmed route changes localises the mechanics: divergence concentrates at hops
3--5---the domestic aggregation and pre-gateway layer, the exact segment an exchange would
replace---detours toggle on and off symmetrically rather than migrating (96\% of changing pairs
return to a previously observed route), and 49\% of all pairs genuinely re-route at the AS level
at least once during the week (a broader population and a looser per-pair test than the
233-pair figure above; the two are not the same measurement). The tromboning rate is therefore
best stated as a stable weekly range, 14.5--15.9\%, produced not by a fixed set of broken routes
but by routes that some of the time genuinely change carrier, and some of the time only appear to
because of measurement noise on an unchanged path---both of which we can now tell apart.
...
exchange would; and since, for the population whose hairpin we can confirm at the route level
rather than from latency alone, the same pairs are served domestically on other rounds of the same
week, the domestic path plainly exists and is simply not always being chosen.
...
that for a third of the probe--site pairs that ever hairpin, the hairpin is confirmed by a change
in the actual route and not just in measured latency, and those same pairs are also served
domestically within the same week, while not one of the week's 222,944 traces crossed either of
the country's exchange fabrics.
