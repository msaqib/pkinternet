...
week---direct evidence that the local path exists and is not taken. Not one of the week's traces
crossed either of the country's exchange fabrics. The finding of the study is that the local path
exists but is not taken, and that PKIX is the instrument that would take it.
\end{abstract}
...
the physical floor, while a quarter of domestic paths do. Third, the \emph{CDN} curve is bimodal:
...
country and returned. The rate is governed far more by the vantage than by the destination: per
...
$+15.3$\,ms across the 121 such pairs, a heavily skewed distribution. Most flips are near-free equal-cost detours; the cost
of tromboning is concentrated in the persistently hairpinned pairs sitting at ${\sim}100$\,ms
against a ${\sim}25$\,ms local norm. The temporal decomposition confirms the penalty is structural
rather than congestive: the hourly tromboning rate varies only from 13.6\% (06:00 PKT) to 16.5\%
(23:00 PKT)---under 3 percentage points around the clock---and the daily rate stays within
14.5--15.9\% across all seven days. A flat profile of this kind shows the penalty to be a
