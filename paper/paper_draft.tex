...
across 17 scheduled probes. Three probes are excluded from analysis. The NTC probe (1016431) was
offline throughout the run and returned no data at all. Probe 1015491 carries an erroneous ASN
label (AS13335, Cloudflare) but is in fact a Z-Com (AS152605) vantage co-located in Lahore with
probe 7613, and is excluded as a mislabelled duplicate. Probe 7764 (PTCL Lahore) recorded 90\%
packet loss throughout the run and is excluded from all RTT and trombone analyses; because a
blinded trace defaults to a non-trombone verdict, retaining this probe would also systematically
understate its ISP's hairpin rate. The remaining 14 probes contribute 204,384 traceroute rows and
313,276 ping rows to analysis. The active probes span nine Pakistani ISPs across seven cities:
Cybernet (Haripur, Karachi $\times$2), Nayatel (Islamabad $\times$2), Nova (Lahore), Orbit
(Faisalabad), PTCL (Karachi, Mianwali), TES/Transworld Home (Rawalpindi, Karachi), Transworld
(Lahore), Fasttel (Islamabad), and Zcom (Lahore), covering both licensed international gateway
operators and seven downstream ISPs.

Among these 14, probe 1016393 (PTCL Mianwali) was connected intermittently---at full rate for the
first two days (11--12 July), then sporadically through the end of the window---and is retained,
flagged wherever it enters per-vantage results.
