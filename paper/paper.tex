...
\begin{table}[t]
  ...
  \begin{tabular}{lcc}
    ...
    \midrule
    Average RTT   & $148 \to 145$\,ms   & $+2\%$ \\
    Jitter        & $13.6 \to 10.3$\,ms & $\mathbf{+31\%}$ \\
    Path length   & $41 \to 44$ hops    & ${\sim}$flat \\
...
the $11\%$ and separate per-host from time-flipping routing; run a 20-day, all-probe
longitudinal panel over CDN/Abroad/PK sites and known tromboning IPs to produce per-ISP RTT
distributions, diurnal and weekly stability, and a normal-day baseline that captures the next
cable fault with a true before/during/after.
